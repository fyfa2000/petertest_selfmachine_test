...
public class BaseRecycleViewAdapter<T, K extends ViewDataBinding> extends RecyclerView.Adapter<BaseViewHolder<K>> {
    ...
    protected int clickBtnId;   //点击控件的ID内VariableId

    public BaseRecycleViewAdapter(List<T> lists, int resouceId, int variableId, int clickBtnId) {
 ...
 private void setBtnClick(final BaseViewHolder<K> holder, final int position){
     holder.getDataBing().getRoot().findViewById(clickBtnId).setOnClickListener(new View.OnClickListener() {
         @Override
         public void onClick (View view){
             // todo
             btnClickListener.OnClick(holder.getDataBing(),position);
         }
